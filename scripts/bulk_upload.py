...
def load_excel(path: str) -> list[dict]:
    ...
    if has_header:
        ...
        # 헤더 행은 있어도 특정 칸만 비어있는 경우, 그 칸의 컬럼명이 빈 문자열이 되어
        # 값이 있어도 어떤 필드인지 인식을 못 하고 유실된다. 이 프로젝트 엑셀들은
        # 공통적으로 category, mc, sku_name, importer, import_type, factory, country[,
        # email] 순서를 따르므로, 빈 헤더 칸은 그 위치의 기본 컬럼명으로 채운다.
        for i, h in enumerate(headers):
            if not h and i < len(_HEADERLESS_COLS):
                headers[i] = _HEADERLESS_COLS[i]
