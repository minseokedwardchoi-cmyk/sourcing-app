...
const styles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero-title { font-size: 20px; font-weight: 700; color: #0f172a; letter-spacing: -0.5px; margin-bottom: 4px; }
  .hero-desc  { font-size: 12px; color: #475569; margin-bottom: 14px; line-height: 1.6; }
  ...
  .hero-kpi-label { font-size: 11px; font-weight: 600; color: #64748b; letter-spacing: 0.3px; margin-bottom: 3px; }
  ...
  return (
    <div className="app">
      ...
      {monthlyModal && (() => {
        ...
        // ③ 연도 범위를 2021부터 현재 연도까지 확장, 없는 해는 0
        const yearlyMap = {};
        ...
        // 데이터가 실제로 존재하는 첫 해 (0이 아닌 첫 연도)
        const firstDataYear = allYears.find(y => y.count > 0)?.year;

        // ② 변화율 계산 helper
        function getYearChangeRate(y, idx) {
          if (y.year === firstDataYear) return null; // 첫 해는 표시 안함
          const prevIdx = allYears.findIndex(a => a.year === String(Number(y.year) - 1));
          if (prevIdx < 0) return null;
          const prev = allYears[prevIdx].count;
          if (prev === 0) return null; // 전년 0이면 표시 안함
          const isCurrent = Number(y.year) === thisYear;
          if (isCurrent) {
            // 전년 동기: 올해 1~thisMonth 합산 vs 작년 1~thisMonth 합산
            const toMonthStr = m => (m < 10 ? "0"+m : String(m));
                      ...
                      <div style={{display:"flex", alignItems:"center", gap:6, fontSize:12}}>
                        ...
                        {(modalChartFrom || modalChartTo) && (
                          <button className="icon-btn" style={{fontSize:11}}
                            onClick={() => { setModalChartFrom(""); setModalChartTo(""); }}>초기화</button>
                        )}
                      </div>
                    </div>
      ...
      <div className="page">
        ...
        <div className="card" style={{padding:16}}>
          ...
          <div style={{position:"relative", border:"1px solid #e8eaed", borderRadius:8, overflow:"hidden", background:"#eff6ff"}}
            ...
            {amountShare?.items?.length > 0 && (
              <div style={{
                position:"absolute", left:10, bottom:70, zIndex:200,
...
function ProductSourcingPage({ navigate }) {
  ...
  return (
    <div className="app">
      ...
      <div className="hero">
        <div className="hero-inner">
          <div className="hero-title">품목별 유통사 인기상품 &amp; 소싱 리스크</div>
          <div className="hero-desc">아마존·월마트·샘스클럽·이온몰의 품목별 인기상품과, 병행수입 가능여부·리콜/품질/법적 리스크 판정을 한 테이블에서 비교합니다. 행을 클릭하면 5년내 이슈·비고 상세가 열립니다.</div>
          {allRows && (
            <div className="hero-kpi">
              {[
                ...
                <div key={label} className="hero-kpi-item">
                  <div className="hero-kpi-label">{label}</div>
                  <div className="hero-kpi-num">{Number(val).toLocaleString()}<span className="hero-kpi-unit">{unit}</span></div>
                </div>
              ))}
            </div>
          )}
