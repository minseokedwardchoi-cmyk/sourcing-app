...
async def compute_best_sku_rankings_for_country(
    ...
    # SKU명 개수가 많은 국가(예: 중국 5만+)에서는 SKU 하나당 바인드 파라미터 하나를 쓰는
    # IN (:sk0, :sk1, ...) 방식이 asyncpg/PostgreSQL의 바인드 파라미터 한도(32767개)를
    # 초과하므로, 배열 하나를 통째로 바인딩하는 = ANY(:sku_names) 방식을 사용한다.
    sku_all_counts_stmt = text("""
        SELECT sku_name, manufacturer, SUM(import_count) AS cnt
        FROM sku_factory_mv
        WHERE sku_name = ANY(:sku_names) AND manufacturer IS NOT NULL
        GROUP BY sku_name, manufacturer
    """).bindparams(bindparam("sku_names", type_=ARRAY(String)))
    sku_all_counts_r = await db.execute(sku_all_counts_stmt, {"sku_names": list(sku_names)})
