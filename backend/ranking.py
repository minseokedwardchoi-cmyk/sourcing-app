...
# ─── 랭킹 계산 결과 캐시 ──────────────────────────────────────────────────────
# 이 파일의 모든 계산은 완료된 과거 연도(y1/y2/y3의 앞 1~2개년)까지 포함해 매 요청마다
# import_history 원본 테이블을 다시 GROUP BY 하는데, 과거 연도 데이터는 새 업로드가
# 있기 전까지는 절대 바뀌지 않는다. 그런데도 국가 페이지·제조사 상세 페이지를 열 때마다
# 매번 처음부터 다시 계산해서 응답이 10초 이상 걸리는 주 원인이 되고 있었다.
# 업로드/크롤링 이후 refresh_mvs()가 호출되는 시점에만 clear_ranking_caches()로
# 무효화하고, 그 사이에는 캐시를 그대로 재사용한다(다른 캐시들과 동일한 패턴).
_MAX_CACHE_ENTRIES = 4000


def _lru_get_or_none(cache: OrderedDict, key):
    if key in cache:
...
_factory_rankings_cache: "OrderedDict[tuple, dict]" = OrderedDict()
_factory_ranking_per_sku_cache: "OrderedDict[tuple, dict]" = OrderedDict()
_country_best_sku_rankings_cache: dict[str, dict] = {}


def clear_ranking_caches() -> None:
    _factory_rankings_cache.clear()
    _factory_ranking_per_sku_cache.clear()
    _country_best_sku_rankings_cache.clear()
...
async def compute_factory_rankings(
    ...
    cache_key = tuple(sorted(similar_skus))
    cached = _lru_get_or_none(_factory_rankings_cache, cache_key)
    ...
    result = await _compute_rankings_for_scope(
        db, f"sku_name IN ({in_clause})", in_params, key_col="factory"
    )
    _lru_put(_factory_rankings_cache, cache_key, result)
    return result
...
async def compute_factory_ranking_per_sku(
    ...
    cache_key = (factory, tuple(sorted(skus)))
    cached = _lru_get_or_none(_factory_ranking_per_sku_cache, cache_key)
    if cached is not None:
        return cached
    ...
    _lru_put(_factory_ranking_per_sku_cache, cache_key, results)
    return results
...
async def compute_best_sku_rankings_for_country(
    ...
    """
    cached = _country_best_sku_rankings_cache.get(country)
    if cached is not None:
        return cached
    ...
    if not mfr_sku_counts:
        _country_best_sku_rankings_cache[country] = {}
        return {}
    ...
    for mfr_key, sku_counts_map in mfr_sku_counts.items():
        ...
        import_grade_overall = (
            "A" if weighted_import_grade_score >= 2.5
            else "C" if weighted_import_grade_score < 1.5
            else "B"
        )
    ...
    _country_best_sku_rankings_cache[country] = results
    return results
